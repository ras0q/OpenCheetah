...
namespace gemini {
    ...
    static Code divide_and_round_q_last_ntt_inplace(
        seal::util::RNSIter input,
        size_t input_base_size,
        ...
        inverse_ntt_negacyclic_harvey(
            last_input, rns_ntt_tables[base_q_size - 1]
        );
        ...
        add_poly_scalar_coeffmod(
            last_input, coeff_count, half, last_modulus, last_input
        );

        SEAL_ALLOCATE_GET_COEFF_ITER(temp, coeff_count, pool);
        SEAL_ITERATE(
            iter(
                input, rns_tool->inv_q_last_mod_q(), base_q->base(),
                rns_ntt_tables
            ),
            input_base_size,
            [&](auto I) {
                ...
                if (get<2>(I).value() < last_modulus.value()) {
                    modulo_poly_coeffs(
                        last_input, coeff_count, get<2>(I), temp
                    );
                } else {
                    set_uint(last_input, coeff_count, temp);
                }

                // Lazy subtraction here. ntt_negacyclic_harvey_lazy can take 0
                // < x < 4*qi input.
                uint64_t neg_half_mod =
                    get<2>(I).value() - barrett_reduce_64(half, get<2>(I));

                // Note: lambda function parameter must be passed by reference
                // here
                ...
                SEAL_ITERATE(temp, coeff_count, [&](auto &J) {
                    J -=
                        (qi_lazy & static_cast<uint64_t>(
                                       -static_cast<int64_t>(J >= qi_lazy)
                                   ));
                });
#endif
                ...
                SEAL_ITERATE(iter(get<0>(I), temp), coeff_count, [&](auto J) {
                    get<0>(J) += qi_lazy - get<1>(J);
                });

                // qk^(-1) * ((ct mod qi) - (ct mod qk)) mod qi
                multiply_poly_scalar_coeffmod(
                    get<0>(I), coeff_count, get<1>(I), get<2>(I), get<0>(I)
                );
            }
        );
        return Code::OK;
    ...
    static Code switch_key_inplace(
        ...
        if (!context.using_keyswitching()) {
            ...
            // throw logic_error("keyswitching is not supported by the
            // context");
        }
        ...
        SEAL_ALLOCATE_GET_RNS_ITER(
            t_target, coeff_count, decomp_modulus_size, pool
        );
        ...
        if (is_ntt_form) {
            inverse_ntt_negacyclic_harvey(
                t_target, decomp_modulus_size, key_ntt_tables
            );
        }

        // Temporary result
        auto t_poly_prod(allocate_zero_poly_array(
            key_component_count, coeff_count, rns_modulus_size, pool
        ));

        SEAL_ITERATE(iter(size_t(0)), rns_modulus_size, [&](auto I) {
            size_t key_index =
                (I == decomp_modulus_size ? key_modulus_size - 1 : I);

            // Product of two numbers is up to 60 + 60 = 120 bits, so we can sum
            // up to 256 of them without reduction.
            ...
            auto t_poly_lazy(allocate_zero_poly_array(
                key_component_count, coeff_count, 2, pool
            ));

            // Semantic misuse of PolyIter; this is really pointing to the data
            // for a single RNS factor
            PolyIter accumulator_iter(t_poly_lazy.get(), 2, coeff_count);

            // Multiply with keys and perform lazy reduction on product's
            // coefficients
            SEAL_ITERATE(iter(size_t(0)), decomp_modulus_size, [&](auto J) {
                ...
                // Perform RNS-NTT conversion
                    ...
                    // Perform RNS conversion (modular reduction)
                        ...
                        modulo_poly_coeffs(
                            t_target[J], coeff_count, key_modulus[key_index],
                            t_ntt
                        );
                    }
                    // NTT conversion lazy outputs in [0, 4q)
                    ntt_negacyclic_harvey_lazy(
                        t_ntt, key_ntt_tables[key_index]
                    );
                    t_operand = t_ntt;
                }

                // Multiply with keys and modular accumulate products in a lazy
                // fashion
                SEAL_ITERATE(
                    iter(key_vector[J].data(), accumulator_iter),
                    key_component_count,
                    [&](auto K) {
                        if (!lazy_reduction_counter) {
                            SEAL_ITERATE(
                                iter(
                                    t_operand, get<0>(K)[key_index], get<1>(K)
                                ),
                                coeff_count,
                                [&](auto L) {
                                    unsigned long long qword[2]{0, 0};
                                    multiply_uint64(
                                        get<0>(L), get<1>(L), qword
                                    );

                                    // Accumulate product of t_operand and
                                    // t_key_acc to t_poly_lazy and reduce
            ...
            // appropriate modulus
            PolyIter t_poly_prod_iter(
                t_poly_prod.get() + (I * coeff_count), coeff_count,
                rns_modulus_size
            );
            ...
            SEAL_ITERATE(
                ...
                [&](auto K) {
                    if (lazy_reduction_counter ==
                        ...
                        SEAL_ITERATE(
                            iter(get<0>(K), *get<1>(K)), coeff_count,
                            [&](auto L) {
        ...
        PolyIter t_poly_prod_iter(
            t_poly_prod.get(), coeff_count, rns_modulus_size
        );
        ...
        SEAL_ITERATE(
            ...
            [&](auto I) {
                // ntt form -> ntt form
                divide_and_round_q_last_ntt_inplace(
                    get<1>(I), decomp_modulus_size, key_ntt_tables, rns_tool,
                    pool
                );
                if (!is_ntt_form) {
                    inverse_ntt_negacyclic_harvey(
                        get<1>(I), decomp_modulus_size, key_ntt_tables
                    );
                }
                add_poly_coeffmod(
                    get<0>(I), get<1>(I), decomp_modulus_size, key_modulus,
                    get<0>(I)
                );
            }
        );
    ...
    Code apply_galois_inplace(
        ...
        // Use key_context_data where permutation tables exist since previous
        // runs.
        ...
        if (!encrypted.is_ntt_form()) {
            ...
            galois_tool->apply_galois(
                encrypted_iter[0], coeff_modulus_size, galois_elt,
                coeff_modulus, temp
            );
            ...
            galois_tool->apply_galois(
                encrypted_iter[1], coeff_modulus_size, galois_elt,
                coeff_modulus, temp
            );
        } else {
            // !!! DO NOT CHANGE EXECUTION ORDER!!!
            ...
            galois_tool->apply_galois_ntt(
                encrypted_iter[0], coeff_modulus_size, galois_elt, temp
            );
            ...
            // Next transform encrypted.data(1)
            galois_tool->apply_galois_ntt(
                encrypted_iter[1], coeff_modulus_size, galois_elt, temp
            );
        }
        ...
        Code ok = Code::OK;
        if (std::any_of(
                *temp, *temp + coeff_modulus_size * coeff_count,
                [](uint64_t u) { return u > 0; }
            )) {
            ok = switch_key_inplace(
                encrypted, temp, static_cast<const KSwitchKeys &>(galois_keys),
