...
namespace gemini {
    ...
    static TensorShape GetConv2DOutShape(const HomConv2DSS::Meta &meta) {
        auto o = shape_inference::Conv2D(
            meta.ishape, meta.fshape, meta.padding, meta.stride
        );
        if (!o) {
        ...
        void sub_poly_inplace(
            ...
            for (size_t l = 0; l < L; ++l) {
                seal::util::sub_poly_coeffmod(
                    dst_ptr, src_ptr, n, coeff_modulus[l], dst_ptr
                );
                dst_ptr += n;
                src_ptr += n;
            }
    ...
    void remove_unused_coeffs(
        ...
        if (std::any_of(
                used_indices.begin(), used_indices.end(),
        ...
        for (size_t index = 0; index < N; ++index) {
            ...
            if (std::binary_search(
                    used_indices.cbegin(), used_indices.cend(), index
                )) {
                continue;
            }
    ...
    static void set_poly_coeffs_uniform(
        ...
        for (size_t j = 1; j < coeff_mod_count; ++j) {
            ...
            for (size_t i = 0; i < coeff_count; ++i, ++src_ptr) {
                *dst_ptr++ =
                    barrett_reduce_64(*src_ptr & bitlen_mask, coeff_modulus[j]);
            }
        }
        ...
        for (size_t i = 0; i < coeff_count; ++i, ++dst_ptr) {
            *dst_ptr =
                barrett_reduce_64(*dst_ptr & bitlen_mask, coeff_modulus[0]);
        }
    }

#if USE_APPROX_RESHARE
    Code sample_random_mask(
        const std::vector<size_t> &targets,
        uint64_t *coeffs_buff,
        size_t buff_size,
        seal::Plaintext &mask,
        ...
        if (std::any_of(targets.begin(), targets.end(), [N](size_t c) {
                return c >= N;
            })) {
    ...
    Code sample_random_mask(
        ...
        if (IsTwoPower(t.value())) {
            ...
            std::transform(
                mask.data(), mask.data() + mask.coeff_count(), mask.data(),
                [mod_mask](uint64_t u) { return u & mod_mask; }
            );
        } else {
            // TODO(wen-jie): to use reject sampling to obtain uniform in [0,
            // t).
    ...
    static void asymmetric_encrypt_zero(
        ...
        for (size_t i = 0; i < coeff_modulus_size; i++) {
            ntt_negacyclic_harvey_lazy(
                u.get() + i * coeff_count, ntt_tables[i]
            );
            for (size_t j = 0; j < encrypted_size; j++) {
                ...
                if (!is_ntt_form) {
                    inverse_ntt_negacyclic_harvey(
                        destination.data(j) + i * coeff_count, ntt_tables[i]
                    );
                }
            }
        }
...
#if USE_APPROX_RESHARE
        // NOTE(wen-jie) we skip e[0] here since e[0] is replaced by the secret
        // sharing random.
        ...
        for (size_t j = 0; j < encrypted_size; j++) {
            ...
            for (size_t i = 0; i < coeff_modulus_size; i++) {
                ...
                add_poly_coeffmod(
                    u.get() + i * coeff_count,
                    destination.data(j) + i * coeff_count, coeff_count,
                    coeff_modulus[i], destination.data(j) + i * coeff_count
                );
            }
        }
    ...
    void flood_ciphertext(
        ...
        set_poly_coeffs_uniform(
            random.get(), std::min(64, noise_len), prng, parms
        );
        ...
        for (size_t i = 0; i < ct.coeff_modulus_size(); i++) {
            seal::util::add_poly_coeffmod(
                rns_ptr, dst_ptr, n, coeff_modulus[i], dst_ptr
            );
            rns_ptr += n;
            dst_ptr += n;
        }
        ...
        asymmetric_encrypt_zero(
            context, pk, ct.parms_id(), ct.is_ntt_form(), prng, zero
        );
    ...
    void truncate_for_decryption(
        ...
        // -> round(m'/Delta) = m The low-end bits of c0, c1 are useless for
        // decryption, and thus we can truncate those bits
        const size_t poly_n = ct.poly_modulus_degree();
        // Delta := floor(p0/t)
        const int n_delta_bits = parms.coeff_modulus()[0].bit_count() -
                                 parms.plain_modulus().bit_count();
        const int one_more_bit =
            IsTwoPower(parms.plain_modulus().value()) ? 0 : 1;
        const uint64_t mask0 = make_bits_mask(n_delta_bits - 1 - one_more_bit);
#if !USE_APPROX_RESHARE || BFV_TRUNCATE_LARGE
        std::transform(
            ct.data(0), ct.data(0) + poly_n, ct.data(0),
        ...
        // Std(|c1|_infty) * Std(|s|_infty) Assume |c1| < B is B-bounded
        // uniform. Then the variance Var(|c1|_infty) = B^2*N/12. We need to
        // make sure the value |c1
        ...
        std::transform(
            ct.data(1), ct.data(1) + poly_n, ct.data(1),
    ...
    uint64_t HomConv2DSS::plain_modulus() const {
        if (context_) {
            return context_->first_context_data()
                ->parms()
                .plain_modulus()
                .value();
        } else {
    ...
    Code HomConv2DSS::setUp(
        ...
        if (sk) {
            if (!seal::is_metadata_valid_for(*sk, *context_)) {
                LOG(WARNING)
                    << "HomConv2DSS: invalid secret key for this SEALContext";
                return Code::ERR_INVALID_ARG;
            }
        ...
        if (pk) {
            if (!seal::is_metadata_valid_for(*pk, *context_)) {
                LOG(WARNING)
                    << "HomConv2DSS: invalid public key for this SEALContext";
                return Code::ERR_INVALID_ARG;
            }
    ...
    Code HomConv2DSS::encryptImage(
        ...
        ENSURE_OR_RETURN(
            img.shape().IsSameSize(meta.ishape), Code::ERR_DIM_MISMATCH
        );

        TensorEncoder::Role encode_role = TensorEncoder::Role::none;
        std::vector<seal::Plaintext> polys;
        CHECK_ERR(
            tencoder_->EncodeImageShare(
                encode_role, img, meta.fshape, meta.padding, meta.stride,
                /*to_ntt*/ false, polys
            ),
            "encryptImage"
        );
    ...
    Code HomConv2DSS::encodeImage(
        ...
        CHECK_ERR(
            tencoder_->EncodeImageShare(
                TensorEncoder::Role::evaluator, img, meta.fshape, meta.padding,
                meta.stride,
    ...
    Code HomConv2DSS::encodeFilters(
        ...
        for (const auto &f : filters) {
            ENSURE_OR_RETURN(
                f.shape().IsSameSize(meta.fshape), Code::ERR_DIM_MISMATCH
            );
        }
        ...
        auto encode_program = [&](long wid, size_t start, size_t end) {
            for (size_t i = start; i < end; ++i) {
                CHECK_ERR(
                    tencoder_->EncodeFilter(
                        filters[i], meta.ishape, meta.padding, meta.stride,
    ...
    size_t HomConv2DSS::conv2DOneFilter(
            ...
            for (size_t i = 0; i < out_size; ++i) {
                ...
                if (out_buff[o].size() > 0) {
                    ...
                    evaluator_->multiply_plain(
                        image.at(ii), filter.at(c), out_buff[o]
                    );
                }
            }
        }
    ...
    Code HomConv2DSS::conv2DSS(
        ...
        ENSURE_OR_RETURN(
            filters.size() == meta.n_filters, Code::ERR_DIM_MISMATCH
        );
        if (meta.is_shared_input) {
            ENSURE_OR_RETURN(
                img_share0.size() == img_share1.size(), Code::ERR_DIM_MISMATCH
            );
        }
        ...
        auto tl_pool = seal::MemoryManager::GetPool(
            seal::mm_prof_opt::mm_force_thread_local
        );
        ...
        auto add_program = [&](long wid, size_t start, size_t end) {
            for (size_t i = start; i < end; ++i) {
                try {
                    evaluator_->add_plain(
                        img_share0[i], img_share1[i], image[i]
                    );
        ...
        ConvCoeffIndexCalculator indexer(
            N, meta.ishape, meta.fshape, meta.padding, meta.stride
        );
        const size_t n_one_channel =
            indexer.slice_size(1) * indexer.slice_size(2);
        ...
        auto conv_program = [&](long wid, size_t start, size_t end) {
            for (size_t m = start; m < end; ++m) {
                ...
                size_t used = conv2DOneFilter(
                    meta.is_shared_input ? image : img_share0, filters[m], meta,
                    ct_start, n_one_channel
                );
        ...
        if (scheme() == seal::scheme_type::bfv) {
            auto truncate_program = [&](long wid, size_t start, size_t end) {
                for (size_t cid = start; cid < end; ++cid) {
                    truncate_for_decryption(
                        out_share0[cid], *evaluator_, *context_
                    );
                }
                return Code::OK;
            };
            ...
            CHECK_ERR(
                LaunchWorks(tpool, out_share0.size(), truncate_program),
                "conv2D"
            );
        }
    ...
    Code HomConv2DSS::sampleRandomMask(
        ...
        return sample_random_mask(
            targets, coeffs_buff, buff_size, mask, pid, prng, *context_
        );
    }

    Code HomConv2DSS::addRandomMask(
        std::vector<seal::Ciphertext> &enc_tensor,
        Tensor<uint64_t> &mask_tensor,
        const Meta &meta,
        size_t nthreads
    ) const {
        ...
        if (!shape_inference::Conv2D(
                meta.ishape, meta.fshape, poly_degree(), meta.padding,
                meta.stride, strided_ishape, pads, slice_width
            )) {
        ...
        auto mask_program = [&](long wid, size_t start, size_t end) {
            ...
            auto prng = context_->first_context_data()
                            ->parms()
                            .random_generator()
                            ->create();
            for (size_t m = start; m < end; ++m) {
                size_t cid = m * n_one_channel;
                for (int sh = 0, hoffset = 0; sh < indexer.slice_size(1);
                     ++sh) {
                    for (int sw = 0, woffset = 0; sw < indexer.slice_size(2);
                         ++sw) {
                        CHECK_ERR(
                            indexer.Get({sh, sw}, slice_shape, targets), "Get"
                        );

                        if (slice_shape.height() * slice_shape.width() !=
                            targets.size()) {
                            return Code::ERR_DIM_MISMATCH;
                        }
                        ...
                        flood_ciphertext(
                            this_ct, prng, *context_, *pk_, *evaluator_
                        );
                        CHECK_ERR(
                            sampleRandomMask(
                                targets, coeffs.data(), coeffs.size(), mask,
                                this_ct.parms_id(), prng, this_ct.is_ntt_form()
                            ),
                            "RandomMaskPoly"
                        );
                        internal::sub_poly_inplace(
                            this_ct, mask, *context_, *evaluator_
                        );

                        auto coeff_ptr = coeffs.data();
                        for (long h = 0; h < slice_shape.height(); ++h) {
                            for (long w = 0; w < slice_shape.width(); ++w) {
                                mask_tensor(m, hoffset + h, woffset + w) =
                                    *coeff_ptr++;
                            }
                        }
    ...
    // coefficients.
    Code HomConv2DSS::removeUnusedCoeffs(
        std::vector<seal::Ciphertext> &ct, const Meta &meta, double *density
    ) const {
        TensorShape out_shape = GetConv2DOutShape(meta);
        ...
        std::array<int, 3> slice_width{0};
        if (!shape_inference::Conv2D(
                meta.ishape, meta.fshape, N, meta.padding, meta.stride,
        ...
        ConvCoeffIndexCalculator indexer(
            N, strided_ishape, meta.fshape, Padding::VALID, 1
        );
        const size_t one_channel =
            indexer.slice_size(1) * indexer.slice_size(2);
        ...
        for (long m = 0; m < meta.n_filters; ++m) {
            ...
            for (int sh = 0; sh < indexer.slice_size(1); ++sh) {
                for (int sw = 0; sw < indexer.slice_size(2); ++sw) {
                    CHECK_ERR(
                        indexer.Get({sh, sw}, slice_shape, indices), "Get"
                    );
    ...
    Code HomConv2DSS::decryptToTensor(
        const std::vector<seal::Ciphertext> &enc_tensor,
        const Meta &meta,
            ...
            for (size_t m = start; m < end; ++m) {
                ...
                for (int sh = 0, hoffset = 0; sh < indexer.slice_size(1);
                    ...
                    for (int sw = 0, woffset = 0; sw < indexer.slice_size(2);
                        ...
                        CHECK_ERR(
                            postProcessInplace(
                                pt, indices, coeffs.data(), coeffs.size()
                            ),
                        ...
                        for (long h = 0; h < slice_shape.height(); ++h) {
                            for (long w = 0; w < slice_shape.width(); ++w) {
                                out_tensor(m, hoffset + h, woffset + w) =
                                    *coeff_ptr++;
                            }
                        }
    ...
    Code HomConv2DSS::postProcessInplace(
        seal::Plaintext &pt,
        std::vector<size_t> &targets,
        uint64_t *out_poly,
        ...
        ENSURE_OR_RETURN(
            out_poly && out_buff_size >= len && len <= N, Code::ERR_DIM_MISMATCH
        );
    ...
    Code HomConv2DSS::idealFunctionality(
        ...
        ENSURE_OR_RETURN(
            meta.ishape.IsSameSize(in_tensor.shape()), Code::ERR_DIM_MISMATCH
        );
        ENSURE_OR_RETURN(
            meta.n_filters == filters.size(), Code::ERR_DIM_MISMATCH
        );
        for (const auto &f : filters) {
            ENSURE_OR_RETURN(
                meta.fshape.IsSameSize(f.shape()), Code::ERR_DIM_MISMATCH
            );
        }
        ...
        for (size_t m = 0; m < meta.n_filters; ++m) {
            ...
            if (base_mod > 1) {
                // mod p
                in_tensor.Conv2D(
                    filters[m], meta.stride, meta.padding, one_channel, base_mod
                );
            } else {
                // mod 2^64
                in_tensor.Conv2D(
                    filters[m], meta.stride, meta.padding, one_channel
                );
            }
