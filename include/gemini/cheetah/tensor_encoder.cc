...
namespace gemini {
    ...
    struct FilterIndexer : public ConvIndexer {
       public:
        FilterIndexer(
            ...
            // O = HW*(C-1) + W*(h-1) + (h-1)
            begin_ = offset_ * (fshape.channels() - 1) +
                     ishape.width() * (h - 1) + h - 1;
        }
    ...
    Code TensorEncoder::EncodeFilter(
        const U64Tensor &filter,
        ...
        ENSURE_OR_RETURN(
            img_shape.dims() == 3 && fshape.dims() == 3, Code::ERR_DIM_MISMATCH
        );
        ...
        if (!shape_inference::Conv2D(
                img_shape, fshape, poly_degree(), padding, stride,
        ...
        Conv2DSliceHelper<U64Tensor> helper(
            &filter, fshape, fshape, slice_strides,
            /*padding*/ {0, 0}
        );
        ...
        for (int c = 0; c < helper.num_slices(); ++c) {
            ...
            CHECK_ERR(
                Encode(
                    sliced_ishape, sliced_fshape, sliced_filter, indexer,
                    tmp_buf.get(), poly_degree()
                ),
                "Encode"
            );
            CHECK_ERR(
                A2H(tmp_buf.get(), poly_degree(), out.at(c), Role::evaluator,
                    is_ntt_form),
                "A2H"
            );
        }
    ...
    Code TensorEncoder::EncodeImageShare(
        Role role,
        const U64Tensor &img_tensor,
        ...
        ENSURE_OR_RETURN(
            img_tensor.dims() == 3 && filter_shape.dims() == 3,
        ...
        if (!shape_inference::Conv2D(
                img_tensor.shape(), filter_shape, poly_degree(), padding,
                stride, strided_tshape, paddings, slice_strides
            )) {
        ...
        Conv2DSliceHelper helper(
            &strided_img, strided_img.shape(), filter_shape, slice_strides,
            paddings
        );
        ...
        for (size_t i = 0; i < n_slices; ++i) {
            ...
            CHECK_ERR(
                Encode(
                    si_shape, sf_shape, sliced_padded_img, indexer,
                    tmp_buf.get(), poly_degree()
                ),
                "Encode"
            );
            ...
            CHECK_ERR(
                A2H(tmp_buf.get(), poly_degree(), out[i], role, is_ntt_form),
                "A2H"
            );
        }
        return Code::OK;
    ...
    Code TensorEncoder::Encode(
        ...
        ENSURE_OR_RETURN(
            fshape.IsValid() && ishape.IsValid(), Code::ERR_INVALID_ARG
        );
        ENSURE_OR_RETURN(
            fshape.channels() == ishape.channels(), Code::ERR_DIM_MISMATCH
        );
        ENSURE_OR_RETURN(
            tensor.shape().IsSameSize(ishape) ||
                tensor.shape().IsSameSize(fshape),
            Code::ERR_DIM_MISMATCH
        );
    ...
    Code TensorEncoder::A2HBFV(
        const U64 *vec, size_t len, RLWEPt &pt, const Role role, bool is_to_ntt
    ) const {
    ...
    ConvCoeffIndexCalculator::ConvCoeffIndexCalculator(
        size_t poly_degree,
        TensorShape ishape,
        TensorShape fshape,
        Padding padding,
        ...
        std::array<int, 3> slice_strides;
        if (!shape_inference::Conv2D(
                ishape, fshape, poly_degree, padding, stride, strided_ishape,
        ...
        helper_.reset(new Conv2DSliceHelper<U64Tensor>(
            is_input_compressed ? strided_ishape : ishape, fshape,
    ...
    Code ConvCoeffIndexCalculator::Get(
    ...
    Code ConvCoeffIndexCalculator::Get(
        ...
        ENSURE_OR_RETURN(
            coords[0] >= 0 && coords[0] < slice_size(1), Code::ERR_INVALID_ARG
        );
        ENSURE_OR_RETURN(
            coords[1] >= 0 && coords[1] < slice_size(2), Code::ERR_INVALID_ARG
        );

        TensorShape sliced_shape;
        CHECK_ERR(
            helper_->slice({0, coords[0], coords[1]}, sliced_shape), "slice"
        );
        ...
        if (indices) {
            ...
            for (int h = 0; h < out_dims[0]; ++h) {
                for (int w = 0; w < out_dims[1]; ++w) {
                    *dst_ptr++ = O + img_indexer(
                                         0, h * stride_ + offsets[0],
                                         w * stride_ + offsets[1]
                                     );
                }
            }
