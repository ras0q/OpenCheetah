...
namespace gemini {
    ...
    Code HomFCSS::setUp(
        ...
        if (sk) {
            if (!seal::is_metadata_valid_for(*sk, *context_)) {
                LOG(WARNING)
                    << "HomFCSS: invalid secret key for this SEALContext";
        ...
        if (pk) {
            if (!seal::is_metadata_valid_for(*pk, *context_)) {
                LOG(WARNING)
                    << "HomFCSS: invalid public key for this SEALContext";
                return Code::ERR_INVALID_ARG;
            }
    ...
    Code HomFCSS::encryptInputVector(
        ...
        ENSURE_OR_RETURN(
            input_vector.shape().IsSameSize(meta.input_shape),
            Code::ERR_DIM_MISMATCH
        );
        ENSURE_OR_RETURN(
            meta.input_shape.num_elements() > 0, Code::ERR_INVALID_ARG
        );
        ...
        for (size_t i = 0; i < nout && !is_failed; ++i) {
            ...
            auto end = std::min<size_t>(
                input_vector.length(), start + split_shape.cols()
            );
            auto len = end - start;
            // reversed ordering for the vector
            tmp[0] = input_vector(start);
            std::transform(
                input_vector.data() + start + 1, input_vector.data() + end,
                tmp.rbegin(),
            ...
            if (Code::OK !=
                vec2Poly(tmp.data(), tmp.size(), pt, encode_role, false)) {
                is_failed = true;
            } else {
                try {
    ...
    Code HomFCSS::encodeInputVector(
        const Tensor<uint64_t> &input_vector,
        const Meta &meta,
        ...
        auto encode_prg = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t i = start; i < end && !is_failed; ++i) {
                ...
                if (Code::OK != vec2Poly(
                                    tmp.data(), tmp.size(), encoded_share.at(i),
                                    encode_role, false
                                )) {
                    is_failed = true;
                }
        ...
        auto encode_prg = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t r_blk = start; r_blk < end && !is_failed; ++r_blk) {
                ...
                for (size_t c_blk = 0; c_blk < n_col_blks; ++c_blk) {
                    ...
                    auto col_extent =
                        std::min<size_t>(
                            top_left_col + split_shape.cols(), ncols
                        ) -
                        top_left_col;
                    // Encode the sub-matrix start ad (top_left_row,
                    // top_left_col) with sizes (row_extent, col_extent) Matrix
                    // is stored in row-major
                    auto src_ptr = weight_matrix.data() + top_left_row * ncols +
                                   top_left_col;
                    auto dst_ptr = tmp.begin();
                    for (size_t r = 0; r < row_extent; ++r) {
                        std::copy_n(src_ptr, col_extent, dst_ptr);
                        // For the right-most submatrtix, we might need
                        // zero-padding.
                    ...
                    if (Code::OK != vec2Poly(
                                        tmp.data(), tmp.size(),
                                        encoded_share.at(r_blk).at(c_blk),
                                        Role::none, false
                                    )) {
                        is_failed = true;
                        break;
    ...
    Code HomFCSS::matVecMul(
        const std::vector<std::vector<seal::Plaintext>> &matrix,
        ...
        for (const auto &rows : matrix) {
            for (const auto &submat : rows) {
                if (submat.is_zero()) {
                    LOG(WARNING
                    ) << "matVecMul: sub-matrix with all zero is not supported.\
            ...
            auto add_prg = [&](long wid, size_t start, size_t end) {
                for (size_t i = start; i < end; ++i) {
                    evaluator_->add_plain(
                        vec_share0[i], vec_share1[i], input[i]
                    );
                }
                return Code::OK;
            };
        ...
        auto fma_prg = [&](long wid, size_t start, size_t end) {
            for (size_t j = start; j < end; ++j) {
                evaluator_->multiply_plain(
                    meta.is_shared_input ? input[0] : vec_share0[0],
                ...
                for (size_t i = 1; i < n_ct_in; ++i) {
                    seal::Ciphertext tmp;
                    evaluator_->multiply_plain(
                        meta.is_shared_input ? input[i] : vec_share0[i],
                        matrix[j][i], tmp
                    );
    ...
    Code HomFCSS::addRandomMask(
        std::vector<seal::Ciphertext> &cts,
        Tensor<uint64_t> &mask_vector,
        const Meta &meta,
        gemini::ThreadPool &tpool
    ) const {
        ...
        auto mask_prg = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t r_blk = start; r_blk < end; ++r_blk) {
                ...
                auto row_end = std::min<size_t>(
                    row_bgn + split_shape.rows(), meta.weight_shape.rows()
                );
            ...
            seal::util::seal_memzero(
                coeffs.data(), sizeof(uint64_t) * coeffs.size()
            );
            seal::util::seal_memzero(
                mask.data(), sizeof(uint64_t) * mask.coeff_count()
            );
            return Code::OK;
        };
    ...
    // coefficients.
    Code HomFCSS::removeUnusedCoeffs(
        std::vector<seal::Ciphertext> &cts, const Meta &meta, double *density
    ) const {
    ...
    Code HomFCSS::decryptToVector(
        const std::vector<seal::Ciphertext> &enc_vector,
        const Meta &meta,
        Tensor<uint64_t> &out,
        size_t nthreads
    ) const {
    ...
    Code HomFCSS::idealFunctionality(
        const Tensor<uint64_t> &input_matrix,
        const Tensor<uint64_t> &weight_matrix,
        const Meta &meta,
        Tensor<uint64_t> &out
    ) const {
