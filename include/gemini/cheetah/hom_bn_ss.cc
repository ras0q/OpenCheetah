...
namespace gemini {
    ...
    Code HomBNSS::setUp(
        ...
        if (sk) {
            if (!seal::is_metadata_valid_for(*sk, *direct_context_)) {
                LOG(WARNING)
                    << "HomBNSS: invalid secret key for this SEALContext";
                return Code::ERR_INVALID_ARG;
            }
        ...
        if (pk) {
            if (!seal::is_metadata_valid_for(*pk, *direct_context_)) {
                LOG(WARNING)
                    << "HomBNSS: invalid public key for this SEALContext";
                return Code::ERR_INVALID_ARG;
            }
    ...
    Code HomBNSS::setUp(
        ...
        ENSURE_OR_RETURN(
            sks.empty() || contexts.size() == sks.size(), Code::ERR_CONFIG
        );
        ...
        for (size_t i = 0; i < nCRT; ++i) {
            ...
            if (!sks.empty()) {
                ENSURE_OR_RETURN(
                    sks[i] && seal::is_valid_for(*sks[i], contexts[i]),
                    Code::ERR_CONFIG
                );
            }
            ...
            if (i == 0) {
                ...
                ENSURE_OR_RETURN(
                    degree == parms.poly_modulus_degree(), Code::ERR_CONFIG
                );
            }
        ...
        seal::SEALContext crt_context(
            crt_parms, true, seal::sec_level_type::none
        );
        ...
        if (!is_mod_2k) {
            if (nCRT != 1 || primes[0].value() != target_base_mod) {
                LOG(FATAL) << "HomBNSS prime field base_mod mismatch "
                           << primes[0].value() << " != " << target_base_mod
                           << " nCRT " << nCRT;
                return Code::ERR_INVALID_ARG;
            }
        } else {
            const int nbits_crt_plain =
                crt_context.key_context_data()->total_coeff_modulus_bit_count(
                ) -
                1;
            const int n_base_mod_bits = Log2(target_base_mod);
            if (nbits_crt_plain < 2 * n_base_mod_bits + 1 + kStatBits) {
                LOG(FATAL) << "HomBNSS require more primes to convert the "
                              "target nbits of "
                              "shares. Require "
                           << 2 * n_base_mod_bits + 1 + kStatBits
                           << " bits of plaintext but got only "
                           << nbits_crt_plain;
                return Code::ERR_INVALID_ARG;
            }
        }
    ...
    Code HomBNSS::encryptVector(
        ...
        ENSURE_OR_RETURN(
            meta.target_base_mod == target_base_mod_, Code::ERR_OUT_BOUND
        );
        ...
        seal::Serializable<seal::Ciphertext> dummy =
            encryptors_[0]->encrypt_zero();
        ...
        auto encrypt_prg = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t k = start; k < end; ++k) {
                ...
                auto vec_pos_end = std::min<size_t>(
                    vec_pos_bgn + sub_vec_len, meta.vec_shape.length()
                );
                auto len = vec_pos_end - vec_pos_bgn;

                std::copy(
                    in_vec.data() + vec_pos_bgn, in_vec.data() + vec_pos_end,
                    tmp.data()
                );
            ...
            seal::util::seal_memzero(
                pt.data(), sizeof(uint64_t) * pt.coeff_count()
            );
            return Code::OK;
        };
        ...
        //      in_vec.data() + end, tmp.data()); std::fill(tmp.begin() + end -
        //      start, tmp.end(), 0); encoders_[i]->encode(tmp, pt); out.at(i *
        //      n_sub_vecs + j) = encryptors_[i]->encrypt_symmetric(pt);
        //    }
        //  }
    ...
    Code HomBNSS::encodeVector(
        ...
        if (in_vec.dims() != 1 || !in_vec.shape().IsSameSize(meta.vec_shape)) {
            LOG(WARNING) << "encodeVector invalid in_vec shape "
                         << in_vec.shape();
        }
        ENSURE_OR_RETURN(
            !contexts_.empty() && !encoders_.empty(), Code::ERR_CONFIG
        );
        ENSURE_OR_RETURN(
            in_vec.dims() == 1 && in_vec.shape().IsSameSize(meta.vec_shape),
            Code::ERR_CONFIG
    ...
    Code HomBNSS::encodeScales(
        const Tensor<uint64_t> &scales,
        const Meta &meta,
        std::vector<seal::Plaintext> &out,
        size_t nthreads
    ) const {
    ...
    Code HomBNSS::bn(
        ...
        const std::vector<seal::Plaintext> &scales,
        const Meta &meta,
        ...
        ENSURE_OR_RETURN(
            !contexts_.empty() && !evaluators_.empty(), Code::ERR_CONFIG
        );
        ...
        auto bn_prg = [&](long wid, size_t start, size_t end) {
            for (size_t i = start; i < end; ++i) {
                ...
                if (meta.is_shared_input) {
                    evaluators_[crt_idx]->add_plain(
                        vec_share0[i], vec_share1[i], out_share0[i]
                    );
                    evaluators_[crt_idx]->multiply_plain_inplace(
                        out_share0[i], scales[i]
                    );
                } else {
                    evaluators_[crt_idx]->multiply_plain(
                        vec_share0[i], scales[i], out_share0[i]
                    );
                }
    ...
    Code HomBNSS::addMaskPrimeField(
        std::vector<seal::Ciphertext> &cts,
        Tensor<uint64_t> &mask,
        const Meta &meta,
        ThreadPool &tpool
    ) const {
    ...
    Code HomBNSS::addMaskRing(
        ...
        auto crt_prng = crt_context_->key_context_data()
                            ->parms()
                            .random_generator()
                            ->create();
        ...
        auto crt_primes =
            crt_context_->key_context_data()->parms().coeff_modulus();
        auto mask_prg = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t i = start; i < end; ++i) {
                ...
                for (size_t d = vec_pos_bgn; d < vec_pos_end; ++d) {
                    ...
                    *slot_dst++ = seal::util::barrett_reduce_128(
                        u128, crt_primes[crt_idx]
                    );
                }
                ...
                truncate_for_decryption(
                    cts[i], *evaluators_[crt_idx], *contexts_[crt_idx]
                );
            }
            return Code::OK;
        };
    ...
    Code HomBNSS::decryptToVector(
        const std::vector<seal::Ciphertext> &in_vec,
        const Meta &meta,
        ...
        ENSURE_OR_RETURN(
            !contexts_.empty() && !sks_.empty() && !encoders_.empty() &&
                crt_context_,
            Code::ERR_CONFIG
        );
        ...
        for (size_t crt = 0; crt < nCRT; ++crt) {
            decryptors[crt] = std::make_shared<seal::Decryptor>(
                *contexts_.at(crt), *sks_.at(crt)
            );
        }
        ...
        auto decrypt_prg = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t cid = start; cid < end; ++cid) {
                ...
                std::copy_n(
                    ...
                    tmp.begin() + vec_pos_bgn +
                        crt_idx * meta.vec_shape.length()
                );
            }
            seal::util::seal_memzero(
                slots.data(), slots.size() * sizeof(uint64_t)
            );
            seal::util::seal_memzero(
                pt.data(), pt.coeff_count() * sizeof(uint64_t)
            );
            return Code::OK;
        };
        ThreadPool tpool(nthreads);
        ...
        if (nCRT > 1) {
            ...
            rns_tool->base_q()->compose_array(
                tmp.data(), meta.vec_shape.length(), tl
            );
            const uint64_t mod_mask = (1UL << n_base_mod_bits_) - 1;
            for (long i = 0, j = 0; i < meta.vec_shape.length();
                 ++i, j += nCRT) {
    ...
    Code HomBNSS::vec2PolyBFV(
        const uint64_t *vec, size_t len, seal::Plaintext &pt, bool is_to_ntt
    ) const {
    ...
    Code HomBNSS::encodeTensor(
        ...
        ENSURE_OR_RETURN(direct_context_, Code::ERR_CONFIG);
        ENSURE_OR_RETURN(
            in_tensor.shape().IsSameSize(meta.ishape), Code::ERR_CONFIG
        );
        ...
        auto encode_prg = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t cid = start; cid < end; ++cid) {
                ...
                for (int d = 0; d < 3; ++d) {
                    offsets[d] =
                        static_cast<int>(indices[d] * split_shape.dim_size(d));
                }

                SlicedPaddedTensor<Tensor<uint64_t>> tensor_slice(
                    &in_tensor, offsets, split_shape
                );
                ...
                auto code =
                    vec2PolyBFV(tmp.data(), tmp.size(), out.at(cid), false);
                if (code != Code::OK) {
                    LOG(FATAL)
                        << "vec2PolyBFV: " << CodeMessage(code) << std::endl;
                }
            }
            seal::util::seal_memzero(tmp.data(), tmp.size() * sizeof(uint64_t));
            return Code::OK;
    ...
    Code HomBNSS::encryptTensor(
        ...
        ENSURE_OR_RETURN(
            direct_context_ && direct_encryptor_, Code::ERR_CONFIG
        );
        ENSURE_OR_RETURN(
            in_tensor.shape().IsSameSize(meta.ishape), Code::ERR_CONFIG
        );
    ...
    Code HomBNSS::bn_direct(
        const std::vector<seal::Ciphertext> &tensor_share0,
        ...
        ENSURE_OR_RETURN(
            direct_context_ && direct_evaluator_, Code::ERR_CONFIG
        );
        ...
        auto add_prg = [&](long wid, size_t start, size_t end) {
            if (meta.is_shared_input) {
                for (size_t i = start; i < end; ++i) {
                    direct_evaluator_->add_plain(
                        tensor_share0[i], tensor_share1[i], out_share0[i]
                    );
                }
            } else {
                for (size_t i = start; i < end; ++i) {
        ...
        auto mul_prog = [&](long wid, size_t start, size_t end) {
            for (size_t c = start; c < end; ++c) {
                ...
                uint64_t v = barrett_reduce_64(
                    static_cast<uint64_t>(std::abs(s)), mod_plain
                );
                ...
                for (size_t cid = cid_start; cid < cid_end; ++cid) {
                    auto cntxt = direct_context_->get_context_data(
                        out_share0[cid].parms_id()
                    );
                    if (!cntxt) {
                    ...
                    for (size_t k = 0; k < out_share0[cid].size(); ++k) {
                        ...
                        for (size_t l = 0; l < modulus.size(); ++l) {
                            ...
                            multiply_poly_scalar_coeffmod(
                                rns_ptr, poly_degree(), scalar, modulus[l],
                                rns_ptr
                            );
                            rns_ptr += poly_degree();
                        }
        ...
        auto mask_prog = [&](long wid, size_t start, size_t end) {
            ...
            for (size_t cid = start; cid < end; ++cid) {
                ...
                direct_pk_encryptor_->encrypt_zero(
                    out_share0[cid].parms_id(), zero
                );
                ...
                remove_unused_coeffs(
                    out_share0[cid], *direct_evaluator_, used_coeff_indices
                );

                truncate_for_decryption(
                    out_share0[cid], *direct_evaluator_, *direct_context_
                );
                seal::util::seal_memzero(
                    rnd[cid].data(), rnd[cid].coeff_count() * sizeof(uint64_t)
                );
            }
            return Code::OK;
        };
        ...
        auto &parms =
            direct_context_->get_context_data(out_share0[0].parms_id())
                ->parms();
        auto prng = parms.random_generator()->create();
        const size_t nbytes =
            mul_safe<size_t>(meta.ishape.num_elements(), sizeof(uint64_t));
        prng->generate(
            nbytes, reinterpret_cast<std::byte *>(out_share1.data())
        );
        ...
        if (IsTwoPower(target_base_mod_)) {
            ...
            modulo_poly_coeffs(
                out_share1.data(), out_share1.NumElements(), mod_plain,
                out_share1.data()
            );
        }
    ...
    Code HomBNSS::decryptToTensor(
        const std::vector<seal::Ciphertext> &cts,
        const Meta &meta,
                ...
                for (int c = 0; c < split_shape.channels(); ++c) {
                    for (int h = 0; h < split_shape.height(); ++h) {
                        for (int w = 0; w < split_shape.width(); ++w) {
                            ...
                            out_tensor(
                                offsets[0] + c, offsets[1] + h, offsets[2] + w
                            ) = *pt_ptr++;
                        }
                    }
                }
