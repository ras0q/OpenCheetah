    ...
    namespace shape_inference {
        ...
        bool MakeSamePadShape(
            ...
            for (int d = 1; d < tshape.dims(); ++d) {
                padded_shape.Update(
                    d, tshape.dim_size(d) + fshape.dim_size(d) - 1
                );
            }
        ...
        std::optional<TensorShape> Conv2D(
            ...
            if (padding == Padding::SAME) {
                if (!shape_inference::MakeSamePadShape(
                        tshape, fshape, oshape
                    )) {
                    return std::nullopt;
                }
            } else {
        ...
        bool Conv2D(
            ...
            const int Cw =
                poly_degree / (filter_shape.height() * filter_shape.width());
            if (Cw <= 0) {
            ...
            for (int d = 0; d < filter_shape.dims(); ++d) {
                if (tshape.dim_size(d) < filter_shape.dim_size(d)) {
                    LOG(WARNING) << "Conv2D: tensor_shape.dim_size(d) < "
                                    "filter_shape.dim_size(d)";
                    return false;
                }
            }
            ...
            if (HW <= poly_degree) {
                // H * W <= N
                slice_strides[0] =
                    std::min<int>(poly_degree / HW, tshape.dim_size(0));
                ...
                slice_strides[1] =
                    std::min(static_cast<int>(std::sqrt(poly_degree)), H);
                slice_strides[2] = slice_strides[1];
                slice_strides[1] = std::min(
                    static_cast<int>(std::sqrt(poly_degree * ratio)), H
                );
                slice_strides[2] =
                    std::min<int>(poly_degree / slice_strides[1], W);
