...
namespace gemini {
    ...
    class Tensor {
       public:
        using ScalarType = TT;
        using SignedScalarType = typename std::conditional<
            std::is_integral<ScalarType>::value,
            int64_t,
            double>::type;

        static_assert(
            std::is_same<double, ScalarType>::value ||
        ...
        static Tensor<ScalarType> Wrap(
            ScalarType *raw_data, TensorShape shape
        ) {
        ...
        SignedScalarType MaxDiff(const Tensor<ScalarType> &oth) const {
            ...
            for (size_t i = 0; i < shape().num_elements(); ++i) {
                SignedScalarType _expect =
                    static_cast<SignedScalarType>(*ptr0++);
                SignedScalarType _computed =
                    static_cast<SignedScalarType>(*ptr1++);
        ...
        typename TTypes<ScalarType, 1>::ConstTensor vector() const {
            ...
            return typename TTypes<ScalarType, 1>::ConstTensor(
                data(), shape_.length()
            );
        }
        ...
        typename TTypes<ScalarType, 1>::Tensor vector() {
            ...
            return
                typename TTypes<ScalarType, 1>::Tensor(data(), shape_.length());
        }
        ...
        typename TTypes<ScalarType, 2>::ConstTensor matrix() const {
            ...
            return typename TTypes<ScalarType, 2>::ConstTensor(
                data(), shape_.rows(), shape_.cols()
            );
        }
        ...
        typename TTypes<ScalarType, 2>::ConstTensor matrix() {
            ...
            return typename TTypes<ScalarType, 2>::Tensor(
                data(), shape_.rows(), shape_.cols()
            );
        }
        ...
        typename TTypes<ScalarType, 3>::Tensor tensor() {
            ...
            return typename TTypes<ScalarType, 3>::Tensor(
                data(), shape_.dim_size(0), shape_.dim_size(1),
                shape_.dim_size(2)
            );
        }
        ...
        Code Conv2D(
            const Tensor<ScalarType> &filter,
            int stride,
            Padding padding,
            Tensor<ScalarType> &out,
            const seal::Modulus &mod
        ) const {
            ENSURE_OR_RETURN(
                IsValid() && filter.IsValid(), Code::ERR_INVALID_ARG
            );
            ENSURE_OR_RETURN(
                dims() == 3 && filter.dims() == 3, Code::ERR_INVALID_ARG
            );
            ENSURE_OR_RETURN(stride > 0, Code::ERR_INVALID_ARG);
            ENSURE_OR_RETURN(
                dim_size(0) == filter.dim_size(0), Code::ERR_DIM_MISMATCH
            );
            ENSURE_OR_RETURN(
                dim_size(1) >= filter.dim_size(1), Code::ERR_DIM_MISMATCH
            );
            ENSURE_OR_RETURN(
                dim_size(2) >= filter.dim_size(2), Code::ERR_DIM_MISMATCH
            );
            ...
            oshape.Update(0, 1);
            oshape.Update(
                1, (oshape.height() - fshape.height() + stride) / stride
            );
            oshape.Update(
                2, (oshape.width() - fshape.width() + stride) / stride
            );

            out.Reshape(oshape);

            for (long ih = -padh, oh = 0; oh < oshape.height();
                 ih += stride, ++oh) {
                for (long iw = -padw, ow = 0; ow < oshape.width();
                     iw += stride, ++ow) {
                    ScalarType sum[2]{0};
        ...
        void Uniform() {
            if (std::is_same<ScalarType, uint64_t>::value) {
                ...
                std::generate_n(
                    wrapped_raw_data_ ? wrapped_raw_data_ : raw_data_.data(),
        ...
        void Randomize(ScalarType range = 1) {
            SignedScalarType upper =
                std::abs(static_cast<SignedScalarType>(range));
            std::random_device rdv;
