...
namespace gemini {
    ...
    class HomBNSS {
       public:
        ...
        Code sendEncryptVector(IO *io, const CtVecType &ct, const Meta &meta)
            const {
        ...
        template <class IO>
        Code recvEncryptVector(
            IO *io, std::vector<seal::Ciphertext> &ct, const Meta &meta
        ) const {
            TensorShape split_shape = getSplit(meta);
            const size_t nCRT = split_shape.dim_size(0);
            const size_t sub_vec_len = split_shape.dim_size(1);
            ...
            for (size_t i = 0; i < nCRT; ++i) {
                for (size_t j = 0; j < n_sub_vecs; ++j) {
                    ...
                    if (!seal::is_valid_for(ct[cid], *contexts_[i])) {
                        LOG(WARNING)
                            << "bn recvEncryptVector invalid ciphertext";
                    }
                    delete[] c_enc_result;
                }
            }
       ...
       protected:
        ...
        Code
        addMaskPrimeField(std::vector<seal::Ciphertext> &ct, Tensor<uint64_t> &mask, const Meta &meta, ThreadPool &)
            const;
        Code
        addMaskRing(std::vector<seal::Ciphertext> &ct, Tensor<uint64_t> &mask, const Meta &meta, ThreadPool &)
            const;
        Code
        addMask(std::vector<seal::Ciphertext> &ct, Tensor<uint64_t> &mask, const Meta &meta, ThreadPool &)
            const;

        Code initPtx(
            seal::Plaintext &pt, seal::parms_id_type pid = seal::parms_id_zero
        ) const;
        Code vec2PolyBFV(
            const uint64_t *vec, size_t len, seal::Plaintext &pt, bool is_ntt
        ) const;
