    ...
    class Strided3DTensor {
       public:
        ...
        explicit Strided3DTensor(const Base &base, std::array<int, 3> strides3d)
            ...
            for (int d : {0, 1, 2}) {
                shape_.Update(
                    d, CeilDiv<int64_t>(shape_.dim_size(d), strides3d[d])
                );
            }
        }
       ...
       public:
        ...
        ScalarType operator()(int c, int h, int w) const {
            ...
            return base_->operator()(
                c + offsets_[0], (h - hpads_[0]) + offsets_[1],
        ...
        SlicedPaddedTensor(
            ...
            std::array<int, 2> hpads,
            std::array<int, 2> wpads,
       ...
       public:
        explicit Conv2DSliceHelper(
            ...
            : base_(img),
              shape_(ishape),
              padded_shape_(
                  {shape_.dim_size(0), shape_.dim_size(1) + paddings[0],
            ...
            if (std::any_of(paddings_.begin(), paddings_.end(), [](int s) {
                    return s < 0;
                })) {
            ...
            for (int d : {1, 2}) {
                ...
                if (padded_shape_.dim_size(d) > slice_strides_[d]) {
                    ...
                    slices_[d] =
                        1 + CeilDiv<int>(
                                padded_shape_.dim_size(d) - slice_strides_[d],
                                overlap_strides_[d]
                            );
                } else {
                    slices_[d] = CeilDiv<int>(
                        padded_shape_.dim_size(d), slice_strides_[d]
                    );
                }
        ...
        Conv2DSliceHelper(
            ...
            : Conv2DSliceHelper(
                  nullptr, ishape, fshape, slice_strides, paddings
              ) {}
        ...
        Code slice(std::array<int, 3> indices, TensorShape &sliced_shape)
            const {
            ...
            for (int d = 0; d < 3; ++d) {
                ENSURE_OR_RETURN(
                    indices[d] >= 0 && indices[d] < slices_[d],
                    Code::ERR_INVALID_ARG
                );
                int start = indices[d] * overlap_strides_[d];
                int end = std::min<int>(
                    start + slice_strides_[d], padded_shape_.dim_size(d)
                );
        ...
        Code slice(
            std::array<int, 3> indices, SlicedPaddedTensor<Base> &sp_tensor
        ) const {
            ENSURE_OR_RETURN(indices.size() == 3, Code::ERR_DIM_MISMATCH);
            ...
            for (int d = 0; d < 3; ++d) {
                ...
                int clipped_size =
                    std::min<int>(shape_.dim_size(d), p_coord + p_size);
                ...
                pads[d][1] =
                    (p_coord + p_size) - (clipped_coords[d] + clipped_size);
            }

            /// Now we do no pad the dimension(0)
            sp_tensor = SlicedPaddedTensor<Base>(
                base_, clipped_coords, pads[1], pads[2], sliced_shape
            );
            return Code::OK;
        }
