...
void Truncation::truncate(
    ...
    if (apply_msb0_heuristic) {
        ...
        if (party == sci::BOB) {
            ...
            std::transform(inA, inA + dim, adjust.data(), [&](uint64_t x) {
                return (x + big_positive) & mask_bw;
            });

            truncate_msb0(
                dim, adjust.data(), outB, shift, bw, signed_arithmetic
            );
            // Tr(x + 2^m, 2^f) = x/2^f + 2^{m - f}
            uint64_t offset = 1UL << (m - shift);
            std::transform(outB, outB + dim, outB, [&](uint64_t x) {
                return (x - offset) & mask_bw;
            });
        } else {
