...
namespace sci {
    ...
    class OTPack {
       public:
        ...
        /*
         * DISCLAIMER:
         * OTPack copy method avoids computing setup keys for each OT instance
         * by reusing the keys generated (through base OTs) for another OT
         * instance. Ideally, the PRGs within OT instances, using the same keys,
         * should use mutually exclusive counters for security. However, the
            ...
            switch (this->party) {
                case 1:
                    ...
                    this->iknp_straight->setup_send(
                        iknp_s_base->k0, iknp_s_base->s
                    );
                    this->iknp_reversed->setup_recv(
                        iknp_r_base->k0, iknp_r_base->k1
                    );
                    break;
                case 2:
                    for (int i = 0; i < KKOT_TYPES; i++) {
                    ...
                    this->iknp_straight->setup_recv(
                        iknp_s_base->k0, iknp_s_base->k1
                    );
                    this->iknp_reversed->setup_send(
                        iknp_r_base->k0, iknp_r_base->s
                    );
                    break;
            }
            this->do_setup = true;
