    ...
    class SilentOT : public sci::OT<SilentOT<IO>> {
       ...
       public:
        ...
        SilentOT(
            ...
            ferret = new FerretCOT<IO>(
                party, threads, ios, malicious, run_setup, pre_file
            );
            if (warm_up) {
                block tmp;
                ferret->rcot(&tmp, 1);
        ...
        void send_cot(
            uint64_t *data0, const uint64_t *corr, int length, int l
        ) {
        ...
        // Sender chooses one message 'corr'. A correlation is defined by the
        // addition function: f(x) = x + corr Sender receives a random message
        // 'x' as output
        // ('data0').
        void send_ot_cam_cc(
            uint64_t *data0, const uint64_t *corr, int64_t length, int l
        ) {
            ...
            for (int64_t i = 0; i < length; i += ot_bsize) {
                ...
                corrected_y_size = (uint32_t)ceil(
                    (std::min(ot_bsize, length - i) * l) /
                    ((float)sizeof(uint64_t) * 8)
                );
                corrected_bsize = std::min(ot_bsize, length - i);

                sci::pack_cot_messages(
                    y, corr_data, corrected_y_size, corrected_bsize, l
                );
        ...
        void recv_ot_cam_cc(
            uint64_t *data, const bool *b, int64_t length, int l
        ) {
            uint64_t modulo_mask = (1ULL << l) - 1;
            if (l == 64) modulo_mask = -1;
            ...
            for (int64_t i = 0; i < length; i += ot_bsize) {
                ...
                ferret->io->recv_data(
                    recvd, sizeof(uint64_t) * corrected_recvd_size
                );
                ...
                for (int j = i; j < i + ot_bsize and j < length; ++j) {
                    if (b[j])
                        data[j] = (corr_data[j - i] -
                                   _mm_extract_epi64(pad[j - i], 0)) &
                                  modulo_mask;
                    else
                        data[j] =
                            _mm_extract_epi64(pad[j - i], 0) & modulo_mask;
                }
            }
        ...
        void send_ot_cm_cc(
            const block *data0, const block *data1, int64_t length
        ) {
            ...
            for (int64_t i = 0; i < length; i += ot_bsize) {
                ...
                // here, ferret depends on the template parameter "IO", making
                // mitccrh also dependent, hence we have to explicitly tell the
                // compiler that "hash" is a template function. See:
                ...
                ferret->io->send_data(
                    pad, 2 * sizeof(block) * std::min(ot_bsize, length - i)
                );
            }
            delete[] data;
        }
        ...
        void recv_ot_cm_cc(block *data, const bool *r, int64_t length) {
            ...
            for (int64_t i = 0; i < length; i += ot_bsize) {
                ...
                ferret->mitccrh.template hash<ot_bsize, 1>(pad);
                ferret->io->recv_data(
                    res, 2 * sizeof(block) * std::min(ot_bsize, length - i)
                );
        ...
        // chosen message, chosen choice.
        // Here, the 2nd dim of data is always 2. We use T** instead of T*[2] or
        // two arguments of T*, in order to be general and compatible with the
        // API of 1-out-of-N OT.
        template <typename T>
        void send_ot_cm_cc(T **data, int64_t length, int l) {
            ...
            for (int64_t i = 0; i < length; i += ot_bsize) {
                ...
                corrected_y_size = (uint32_t)ceil(
                    (2 * std::min(ot_bsize, length - i) * l) /
                    ((float)sizeof(T) * 8)
                );
                corrected_bsize = std::min(ot_bsize, length - i);

                sci::pack_ot_messages<T>(
                    (T *)y, data + i, pad, corrected_y_size, corrected_bsize, l,
                    2
                );
        ...
        // chosen message, chosen choice
        // Here, r[i]'s value is always 0 or 1. We use uint8_t instead of bool,
        // in order to be general and compatible with the API of 1-out-of-N OT.
        template <typename T>
        void recv_ot_cm_cc(T *data, const uint8_t *r, int64_t length, int l) {
            ...
            for (int64_t i = 0; i < length; i += ot_bsize) {
                ...
                ferret->io->recv_data(
                    recvd, sizeof(T) * (corrected_recvd_size)
                );
                ...
                sci::unpack_ot_messages<T>(
                    data + i, r + i, (T *)recvd, pad, corrected_bsize, l, 2
                );
            }
            delete[] rcm_data;
        }
        ...
        // chosen message, chosen choice.
        // One-oo-N OT, where each message has l bits. Here, the 2nd dim of data
        // is N.
        template <typename T>
        ...
        // chosen message, chosen choice
        // One-oo-N OT, where each message has l bits. Here, r[i]'s value is in
        // [0, N).
        template <typename T>
        void recv_ot_cm_cc(
            T *data, const uint8_t *r, int64_t length, int N, int l
        ) {
            int logN = (int)ceil(log2(N));
            ...
            for (int64_t i = 0; i < length; i += ot_bsize) {
                ...
                sci::unpack_ot_messages<T>(
                    data + i, r + i, (T *)recvd, pad, corrected_bsize, l, N
                );
            }
            delete[] hash_in;
            delete[] hash_out;
