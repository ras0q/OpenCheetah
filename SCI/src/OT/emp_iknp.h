...
namespace emp {
     ...
     *
     * [REF] With optimization of "More Efficient Oblivious Transfer and
     * Extensions for Faster Secure Computation"
     * https://eprint.iacr.org/2013/552.pdf
     * [REF] With optimization of "Better Concrete Security for Half-Gates
     * Garbling (in the Multi-Instance Setting)"
     * https://eprint.iacr.org/2019/1168.pdf
     */
    template <typename IO>
    class IKNP : public COT<IO, IKNP<IO>> {
       public:
        using COT<IO, IKNP<IO>>::io;
        using COT<IO, IKNP<IO>>::Delta;

        OTCO<IO>* base_ot = nullptr;
        ...
        block k0[128], k1[128];
        IKNP(IO* io, bool malicious = false) : malicious(malicious) {
            this->io = io;
        }
        ~IKNP() { delete_array_null(extended_r); }
        ...
        void setup_send(const bool* in_s = nullptr, block* in_k0 = nullptr) {
            ...
            for (int64_t i = 0; i < 128; ++i) G0[i].reseed(&k0[i]);
        ...
        void send_pre(block* out, int64_t length) {
            ...
            if (malicious) send_pre_block(local_out, 256);
        }
        ...
        void send_pre_block(block* out, int64_t len) {
            ...
            for (int64_t i = 0; i < 128; ++i) {
                G0[i].random_data(
                    t + (i * block_size / 128), local_block_size / 8
                );
                if (s[i])
                    xorBlocks_arr(
                        t + (i * block_size / 128), t + (i * block_size / 128),
                        tmp + (i * local_block_size / 128),
        ...
        void recv_pre(block* out, const bool* r, int64_t length) {
            ...
            for (; j < length / block_size; ++j)
                recv_pre_block(
                    out + j * block_size, block_r + (j * block_size / 128),
                    block_size
                );
            int64_t remain = length % block_size;
            if (remain > 0) {
                recv_pre_block(
                    local_out, block_r + (j * block_size / 128), remain
                );
                memcpy(out + j * block_size, local_out, sizeof(block) * remain);
            }
            if (malicious) {
        ...
        void recv_pre_block(block* out, block* r, int64_t len) {
            ...
            for (int64_t i = 0; i < 128; ++i) {
                ...
                xorBlocks_arr(
                    tmp, t + (i * block_size / 128), tmp, local_block_size / 128
                );
        ...
        void recv_cot(block* data, const bool* b, int64_t length) override {
            ...
            if (malicious) recv_check(data, b, length);
        }
        ...
        bool send_check(block* out, int64_t length) {
            ...
            for (int64_t i = 0; i < length / block_size; ++i) {
                ...
                vector_inn_prdt_sum_no_red<block_size>(
                    tmp, chi, out + i * block_size
                );
                q[0] = q[0] ^ tmp[0];
                q[1] = q[1] ^ tmp[1];
            }
            ...
            if (remain != 0) {
                uni_hash_coeff_gen<block_size>(chi, seed2);
                vector_inn_prdt_sum_no_red(
                    tmp, chi, out + length - remain, remain
                );
                q[0] = q[0] ^ tmp[0];
                q[1] = q[1] ^ tmp[1];
            }
        ...
        void recv_check(block* out, const bool* r, int64_t length) {
            block select[2] = {global_zero_block, global_all_one_block};
