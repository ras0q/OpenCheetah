    ...
    class OTCO : public OT<OTCO<IO>> {
       public:
        ...
        void recv_impl(block* data, const bool* b, int64_t length) {
            ...
            Point *B = new Point[length], *As = new Point[length], A;

            for (int64_t i = 0; i < length; ++i) G->get_rand_bn(bb[i]);

            io->recv_pt(G, &A);

            for (int64_t i = 0; i < length; ++i) {
                B[i] = G->mul_gen(bb[i]);
                if (b[i]) B[i] = B[i].add(A);
                io->send_pt(&B[i]);
            }
            io->flush();

            for (int64_t i = 0; i < length; ++i) As[i] = A.mul(bb[i]);
