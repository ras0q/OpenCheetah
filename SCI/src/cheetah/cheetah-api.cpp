...
static void recv_encrypted_vector(
    sci::NetIO *io,
    const seal::SEALContext &context,
    std::vector<seal::Ciphertext> &ct_vec,
    bool is_truncated = false
);
static void recv_ciphertext(
    sci::NetIO *io,
    const seal::SEALContext &context,
    seal::Ciphertext &ct,
    bool is_truncated = false
...
namespace gemini {
    ...
    CheetahLinear::CheetahLinear(
        int party, sci::NetIO *io, uint64_t base_mod, size_t nthreads
    )
        : party_(party), io_(io), nthreads_(nthreads), base_mod_(base_mod) {
        if (base_mod < 2ULL || (int)std::log2(base_mod) >= 45) {
            throw std::logic_error(
                "CheetahLinear: base_mod out-of-bound [2, 2^45)"
            );
        }
        ...
        context_ = std::make_shared<SEALContext>(
            seal_parms, true, sec_level_type::tc128
        );
    ...
    void CheetahLinear::setUpForBN() {
        ...
        for (size_t i = 0; i < nCRT; ++i) {
            ...
            bn_contexts_[i] = std::make_shared<SEALContext>(
                seal_parms, true, sec_level_type::tc128
            );
        }
        ...
        if (party_ == sci::BOB) {
            ...
            if (code != Code::OK) {
                throw std::runtime_error(
                    "BN setUp failed [" + CodeMessage(code) + "]"
                );
            }
        } else {
            bn_pks_.resize(nCRT);
    ...
    bool CheetahLinear::verify(
        const Tensor<uint64_t> &in_tensor_share,
        ...
        if (party_ == sci::BOB) {
            ...
            io_->recv_data(
                in_tensor_raw.data(), sizeof(uint64_t) * in_tensor_raw.size()
            );
            io_->recv_data(
                out_tensor_raw.data(), sizeof(uint64_t) * out_tensor_raw.size()
            );

            auto in_tensor =
                Tensor<uint64_t>::Wrap(in_tensor_raw.data(), ishape);
            auto out_tensor =
                Tensor<uint64_t>::Wrap(out_tensor_raw.data(), oshape);
            ...
            in_tensor.tensor() =
                in_tensor.tensor().unaryExpr([this](uint64_t v) {
                    return reduce(v);
                });
            out_tensor.tensor() =
                out_tensor.tensor().unaryExpr([this](uint64_t v) {
                    return reduce(v);
                });
            ...
            f64_in.tensor() = in_tensor.tensor().unaryExpr([&](uint64_t v) {
                return cast_to_double(v, nbit_precision);
            });
    ...
    void CheetahLinear::fc(
        const Tensor<uint64_t> &input_vector,
        const Tensor<uint64_t> &weight_matrix,
        const FCMeta &meta,
        Tensor<uint64_t> &out_vec_share
    ) const {
        ...
        if (party_ == sci::ALICE &&
            ...
            throw std::invalid_argument(
                "CheetahLinear::fc weight shape mismatch"
            );
        }
        ...
        if (party_ == sci::BOB) {
            {
                ...
                code = impl.encryptInputVector(
                    input_vector, meta, ct_buff, nthreads
                );
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "CheetahLinear::fc encryptInputVector [" +
                        CodeMessage(code) + "]"
                    );
                }
            ...
            if (code != Code::OK) {
                throw std::runtime_error(
                    "CheetahLinear::fc decryptToVector [" + CodeMessage(code) +
                    "]"
                );
            }
        } else {
            std::vector<std::vector<seal::Plaintext>> encoded_matrix;
            code = impl.encodeWeightMatrix(
                weight_matrix, meta, encoded_matrix, nthreads
            );
            if (code != Code::OK) {
                throw std::runtime_error(
                    "CheetahLinear::fc encodeWeightMatrix error [" +
                    CodeMessage(code) + "]"
                );
            }
            std::vector<seal::Plaintext> vec_share1;
            if (meta.is_shared_input) {
                code = impl.encodeInputVector(
                    input_vector, meta, vec_share1, nthreads
                );
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "CheetahLinear::fc encodeInputVector error [" +
                        CodeMessage(code) + "]"
                    );
                }
            }
            ...
            auto code = impl.matVecMul(
                encoded_matrix, vec_share0, vec_share1, meta, out_vec_share0,
                out_vec_share, nthreads
            );
            if (code != Code::OK) {
                throw std::runtime_error(
                    "CheetahLinear::fc matmul2D error [" + CodeMessage(code) +
                    "]"
                );
            }
    ...
    void CheetahLinear::conv2d(
        ...
        if (!meta.ishape.IsSameSize(in_tensor.shape())) {
            throw std::invalid_argument(
                "CheetahLinear::conv2d meta.ishape mismatch"
            );
        }
        ...
        for (const auto &f : filters) {
            if (!meta.fshape.IsSameSize(f.shape())) {
                throw std::invalid_argument(
                    "CheetahLinear::conv2d meta.fshape mismatch"
                );
            }
        }
        ...
        if (party_ == sci::BOB) {
            {
                ...
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "CheetahLinear::conv2d encryptImage " +
                        CodeMessage(code)
                    );
                }
                send_encrypted_vector(io_, ct_buff);
            ...
            if (code != Code::OK) {
                throw std::runtime_error(
                    "CheetahLinear::conv2d decryptToTensor " + CodeMessage(code)
                );
            }
        } else {
            std::vector<std::vector<seal::Plaintext>> encoded_filters;
            code =
                impl.encodeFilters(filters, meta, encoded_filters, nthreads_);
            if (code != Code::OK) {
                throw std::runtime_error(
                    "CheetahLinear::conv2d ecnodeFilters " + CodeMessage(code)
                );
            }
            ...
            if (meta.is_shared_input) {
                code =
                    impl.encodeImage(in_tensor, meta, encoded_share, nthreads_);
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "CheetahLinear::conv2d encodeImage " + CodeMessage(code)
                    );
                }
            }
            ...
            auto code = impl.conv2DSS(
                ct_buff, encoded_share, encoded_filters, meta, out_ct,
                out_tensor, nthreads_
            );
            if (code != Code::OK) {
                throw std::runtime_error(
                    "CheetahLinear::conv2d conv2DSS: " + CodeMessage(code)
                );
            }
    ...
    void CheetahLinear::bn(
        const Tensor<uint64_t> &input_vector,
        const Tensor<uint64_t> &scale_vector,
        const BNMeta &meta,
        Tensor<uint64_t> &out_vector
    ) const {
        if (meta.is_shared_input &&
        ...
        if (party_ == sci::BOB) {
            {
                std::vector<seal::Serializable<seal::Ciphertext>> ct_buff;
                code = bn_impl_.encryptVector(
                    input_vector, meta, ct_buff, nthreads_
                );
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "bn encryptVector [" + CodeMessage(code) + "]"
                    );
                }
                code = bn_impl_.sendEncryptVector(io_, ct_buff, meta);
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "bn sendEncryptVector [" + CodeMessage(code) + "]"
                    );
                }
            }
            ...
            if (code != Code::OK) {
                throw std::runtime_error(
                    "bn recvEncryptVector [" + CodeMessage(code) + "]"
                );
            }

            code =
                bn_impl_.decryptToVector(ct_buff, meta, out_vector, nthreads_);
            if (code != Code::OK) {
                throw std::runtime_error(
                    "bn decryptToVector [" + CodeMessage(code) + "]"
                );
            }
        } else {
            ...
            if (meta.is_shared_input) {
                code = bn_impl_.encodeVector(
                    input_vector, meta, encoded_vector, nthreads_
                );
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "bn encodeVector [" + CodeMessage(code) + "]"
                    );
                }
            }

            code = bn_impl_.encodeScales(
                scale_vector, meta, encoded_scales, nthreads_
            );
            if (code != Code::OK) {
                throw std::runtime_error(
                    "bn encodeScales [" + CodeMessage(code) + "]"
                );
            }
            ...
            std::vector<seal::Ciphertext> out_ct;
            code = bn_impl_.bn(
                encrypted_vector, encoded_vector, encoded_scales, meta, out_ct,
                out_vector, nthreads_
            );
            if (code != Code::OK) {
                throw std::runtime_error(
                    "bn failed [" + CodeMessage(code) + "]"
                );
            }
    ...
    void CheetahLinear::bn_direct(
        const Tensor<uint64_t> &input_tensor,
        const Tensor<uint64_t> &scale_vector,
        const BNMeta &meta,
        Tensor<uint64_t> &out_tensor
    ) const {
        if (meta.is_shared_input &&
            !input_tensor.shape().IsSameSize(meta.ishape)) {
        ...
        if (party_ == sci::BOB) {
            {
                std::vector<seal::Serializable<seal::Ciphertext>> ct_buff;
                code = bn_impl_.encryptTensor(
                    input_tensor, meta, ct_buff, nthreads_
                );
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "bn_direct encryptVector [" + CodeMessage(code) + "]"
                    );
                }
                send_encrypted_vector(io_, ct_buff);
            }
            ...
            code =
                bn_impl_.decryptToTensor(ct_buff, meta, out_tensor, nthreads_);
            if (code != Code::OK) {
                throw std::runtime_error(
                    "bn_direct decryptToTensor [" + CodeMessage(code) + "]"
                );
            }
        } else {
            ...
            if (meta.is_shared_input) {
                code = bn_impl_.encodeTensor(
                    input_tensor, meta, encoded_tensor, nthreads_
                );
                if (code != Code::OK) {
                    throw std::runtime_error(
                        "bn_direct encodeVector [" + CodeMessage(code) + "]"
                    );
                }
            }
            ...
            std::vector<seal::Ciphertext> out_ct;
            code = bn_impl_.bn_direct(
                encrypted_tensor, encoded_tensor, scale_vector, meta, out_ct,
                out_tensor, nthreads_
            );
            if (code != Code::OK) {
                throw std::runtime_error(
                    "bn_direct failed [" + CodeMessage(code) + "]"
                );
            }
            send_encrypted_vector(io_, out_ct);
        }
