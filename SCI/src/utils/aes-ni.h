...
#define KEYEXP256_2(K1, K2) KEYEXP128_H(K1, K2, 0x00, 0xaa)
    ...
    static inline void aes_key_setup_enc(
        block128 rk[], const u8 *cipherKey, int keylen
    ) {
        switch (keylen) {
            case 16: {
                /* 128 bit key setup */
            ...
            case 24: {
                ...
                rk[1] = (block128
                )_mm_shuffle_pd((__m128d)rk[1], (__m128d)temp[0], 0);
                rk[2] = (block128
                )_mm_shuffle_pd((__m128d)temp[0], (__m128d)temp[1], 1);
                ...
                rk[4] = (block128
                )_mm_shuffle_pd((__m128d)rk[4], (__m128d)temp[0], 0);
                rk[5] = (block128
                )_mm_shuffle_pd((__m128d)temp[0], (__m128d)temp[1], 1);
                ...
                rk[7] = (block128
                )_mm_shuffle_pd((__m128d)rk[7], (__m128d)temp[0], 0);
                rk[8] = (block128
                )_mm_shuffle_pd((__m128d)temp[0], (__m128d)temp[1], 1);
                ...
                rk[10] = (block128
                )_mm_shuffle_pd((__m128d)rk[10], (__m128d)temp[0], 0);
                rk[11] = (block128
                )_mm_shuffle_pd((__m128d)temp[0], (__m128d)temp[1], 1);
    ...
    static inline void aes_key_setup_enc(
        block128 rk[], const block256 cipherKey
    ) {
        /* 256 bit key setup */
    ...
    static inline void aes_key_setup_dec(
        block128 dk[], const block128 ek[], int rounds
    ) {
        dk[rounds] = ek[0];
    ...
    static inline void AESNI_set_encrypt_key(
        AESNI_KEY *self, unsigned char *key, int keylen
    ) {
        switch (keylen) {
            case 16:
                self->rounds = 10;
    ...
    static inline void AESNI_set_encrypt_key(
        AESNI_KEY *self, const block256 key
    ) {
        self->rounds = 14;
        aes_key_setup_enc(self->rk, key);
    }

    static inline void AESNI_set_decrypt_key(
        AESNI_KEY *self, unsigned char *key, int keylen
    ) {
        switch (keylen) {
            case 16:
...
#pragma GCC optimize("unroll-loops")
    ...
    static inline void __attribute__((target("aes,sse2")))
    AESNI_ecb_encrypt_blks_ks(
        block128 *blks, unsigned int nblks, const AESNI_KEY *key
    ) {
...
#pragma GCC pop_options
    ...
    static inline void __attribute__((target("aes,sse2")))
    AESNI_ecb_decrypt_blks(
        block128 *blks, unsigned nblks, const AESNI_KEY *key
    ) {
    ...
    /*
    static inline void AESNI_encrypt_label(Label& lb, const AESNI_KEY* key,
    uint64_t gid, uint8_t entry, uint8_t pos){
