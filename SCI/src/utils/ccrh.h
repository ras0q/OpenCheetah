...
namespace sci {
     ...
     * Here we model f(x) = AES_{00..0}(x) as a random permutation (and thus in
     * the RPM model)
     */
    class CCRH : public PRP {
       public:
        CCRH(const block& key = global_zero_block) : PRP(key) {}
...
#pragma GCC optimize("unroll-loops")
        ...
        void H(block out[n], block in[n]) {
            block tmp[n];
            for (int i = 0; i < n; ++i) tmp[i] = out[i] = sigma(in[i]);
            permute_block(tmp, n);
...
#pragma GCC pop_options
        ...
        void Hn(block* out, block* in, int length, block* scratch = nullptr) {
            ...
            for (int i = 0; i < length; ++i) scratch[i] = out[i] = sigma(in[i]);
