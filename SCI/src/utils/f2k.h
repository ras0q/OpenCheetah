...
#ifdef __x86_64__
    __attribute__((target("sse2,pclmul"))) inline void mul128(
        __m128i a, __m128i b, __m128i *res1, __m128i *res2
    ) {
...
/* multiplication in galois field with reduction */
    ...
    inline void vector_inn_prdt_sum_red(
        block *res, const block *a, const block *b, int sz
    ) {
        block r = global_zero_block;
        block r1;
        for (int i = 0; i < sz; i++) {
            gfmul(a[i], b[i], &r1);
            r = r ^ r1;
    ...
    template <int N>
    inline void vector_inn_prdt_sum_red(
        block *res, block const *a, const block *b
    ) {
        vector_inn_prdt_sum_red(res, a, b, N);
    }

    /* inner product of two galois field vectors without reduction */
    inline void vector_inn_prdt_sum_no_red(
        block *res, const block *a, const block *b, int sz
    ) {
        block r1 = global_zero_block, r2 = global_zero_block;
        block r11, r12;
        for (int i = 0; i < sz; i++) {
    ...
    template <int N>
    inline void vector_inn_prdt_sum_no_red(
        block *res, const block *a, const block *b
    ) {
    ...
    inline void uni_hash_coeff_gen(block *coeff, block seed, int sz) {
        ...
        if (remainder != 0) {
            i = sz - remainder;
            for (; i < sz; ++i) gfmul(coeff[i - 1], seed, &coeff[i]);
        }
    }
    ...
    inline void vector_self_xor(block *sum, block *data, int sz) {
        block res[4];
        res[0] = global_zero_block;
        res[1] = global_zero_block;
        res[2] = global_zero_block;
        res[3] = global_zero_block;
