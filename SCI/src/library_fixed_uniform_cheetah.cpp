...
extern void ElemWiseActModelVectorMult_pt(
...
void MatMul2D(
    ...
    for (long r = 0; r < input_shape.rows(); ++r) {
        ...
        if (meta.is_shared_input) {
            input_vector = Tensor<intType>::Wrap(
                const_cast<intType *>(input_row), meta.input_shape
            );
        } else {
            input_vector.Reshape(meta.input_shape);
            std::transform(
                input_row, input_row + meta.input_shape.num_elements(),
                input_vector.data(), [](uint64_t v) { return getRingElt(v); }
            );
        }
        ...
        std::copy_n(
            out_vec.data(), out_vec.shape().num_elements(),
            mat_C + r * input_shape.cols()
        );
    }

    if (cheetah_linear->party() == SERVER) {
        cheetah_linear->safe_erase(
            weight_matrix.data(), meta.weight_shape.num_elements()
        );
    }
#ifdef LOG_LAYERWISE
...
void Conv2DWrapper(
    ...
    if (party == SERVER) {
        ...
        Conv2DWrapper_pt(
            ...
            zPadWRight, strideH, strideW, VinputVec, VfilterVec, VoutputVec
        );
...
void BatchNorm(
    ...
    int32_t W,
    int32_t C,
    const intType *inputArr,
    const intType *scales,
    const intType *bias,
    intType *outArr
) {
        ...
        for (int32_t h = 0; h < H; ++h) {
            for (int32_t w = 0; w < W; ++w) {
                for (int32_t c = 0; c < C; ++c) {
                    in_tensor(c, h, w) = getRingElt(
                        Arr4DIdxRowM(inputArr, B, H, W, C, b, h, w, c)
                    );
                }
            }
        }
...
void ElemWiseActModelVectorMult(
    ...
    static int batchNormCtr = 1;
    printf(
        "HomBN #%d via element-wise mult on %d points\n", batchNormCtr++, size
    );
    ...
    if (cheetah_linear->party() == SERVER) {
        std::transform(
            multArrVec, multArrVec + size, scale_vec.data(), getRingElt
        );
    }
